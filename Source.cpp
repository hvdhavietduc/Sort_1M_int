...
int main() {
	...
	while ( !file.eof()) {
		...
		if (i == n) {
			...
			if (!check_next_file) {
				while (i != j) {
					if(j==n-1){
						output1 << to_string(arr[j])<<endl << "END";
					}
					else output1 << to_string(arr[j]) << endl;
					j++;
				}
				i = 0;
				...
				while (i != j) {
					if (j == n-1) {
						output2 << to_string(arr[j]) <<endl<< "END";
					}
					else output2 << to_string(arr[j]) << endl;
					j++;
				}
				i = 0;
			}
	...
	fstream finalOutput;
	finalOutput.open("finalOutput.txt", ios::out);
	string tmp1, tmp2;
